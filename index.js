...
const vcard = 'BEGIN:VCARD\n' // Tarjeta de contacto
            ...
            + 'FN:Shan\n' // Nombre
            + 'ORG:Shanduy;\n' // Propietario
            + 'TEL;type=CELL;type=VOICE;waid=542252511012:+52 2252 511012\n' // ID de WhatsApp + número de teléfono
            + 'END:VCARD'
...
function addMetadata(packname, author) {	
	if (!packname) packname = 'BotEmolai'; if (!author) author = 'sofi';	
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Opaa que onda @${num.split('@')[0]}\nBienvenido a *${mdata.subject}* el grupo santo de los fieles 👉😎👈\n\nPresentarse con NOMBRE, FOTO, EDAD Y PAIS. Prohibido xxx, spam, gore y hablar privado sin permiso. Si incumplis las normas te vas a narnia pa🧐\n\nPara utilizar el bot registrate con el comando ${prefix}daftar y tu nombre\n\nPara ver los demas comandos utiliza ${prefix}help\n\nby sofi`
                          client.sendMessage(mdata.id, teks, MessageType.text, { contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				teks = `Un infiel menos @${num.split('@')[0]}👋\n\nquien lo va a extrañar a ese`
				client.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					...
					ownerG: '[❗] Este comando solo puede ser utilizado por un dios admin del grupo',
					ownerB: '[❗] Este comando solo lo usa ShanBot',
					admin: '[❗] Este comando solo puede ser utilizado por un dios admi del grupo',
					Badmin: '[❗] Este comando solo se puede usar cuando el bot se convierte en administrador',
                                        pegatina: 'Calma crack estoy haciendo tu sticker 👏\n\n*Recuerda los stickersgif son de 6 segundos❗*\n\nby sofi',
					attp: 'Paraaa vieja estoy haciendo tu stickeer 👏\n\n*Esto puede demorar unos añitos*\n\nby sofi',
					...
					musica: 'Que temazooo amigoo ya te lo descargo pa\n\n*Recorda colocar bien el nombre de la cancion o el link si no no funca vite*\n\n*Si el comando *play no funciona utiliza el comando *play2*\n\nby sofi',
					musica2: 'Que temazooo amigoo ya te lo descargo pa\n\n*Recorda colocar bien el nombre de la cancion o el link si no no funca vite*\n\n*Si el comando *play2 no funciona utiliza el comando *play*\n\nby sofi',
					daftarB: `「NO ESTAS REGISTRADO」\n\nQUIEN TE CONOCE A VO?\n\nPara poder usarme escribe el siguente comando\n\nComando: ${prefix}daftar Nombre\nEjemplo: ${prefix}daftar sofi`,
				}
			}
    			const apakah = ['Si','No']
		...
		if (isGroupAdmins) return reply('Sos mi dios pa, así que no te prohibiré el uso de enlaces :)')
		client.updatePresence(from, Presence.composing)
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`legiovirgo detectado ${sender.split("@")[0]} Pa tu casa pa`)
		setTimeout( () => {
	       ...
	       case 'troleo':
               ...
               break*/
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pedido recibido✅\n\nUn dios menos @${mentioned[0].split('@')[0]}\n*${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                 case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pedido recibido✅\n\nAgregando nuevo dios : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
...
break				
  ...
  client.updatePresence(from, Presence.composing) 
	...
	case 'creador':
		...
		client.sendMessage(from, 'Arriba está el número del creador del bot <BotEmolai by sofi>\n\nAhi puedes resolver tus preguntas y errores :)\n\nEste es el numero de la propietaria y creadora del bot que estas usando\n\nby sofi',MessageType.text, { quoted: mek} )
	...
	case 'hidetag':
              ...
              await client.sendMessage(from, options, text)
               break
					...
					teks = `*DESCARGA EXITOSA ✅*\n◉ *Título* : ${anu.title}\n\n*ESPERA CHE QUE ESTOY MANDANDO EL MP3 ⚠*`
				...
				case 'ytmp4':
					...
					if (anu.error) return reply(anu.error)
					teks = `*DESCARGA EXITOSA ✅*\n◉ *Título* : ${anu.title}\n\n*ESPERA CHE QUE ESTOY MANDANDO EL MP4 ⚠*`
					thumb = await getBuffer(anu.thumb)
					client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
				...
				case 'adminlist':
					client.updatePresence(from, Presence.composing) 
                                        if (!isUser) return reply(mess.only.daftarB)
					if (!isGroup) return reply(mess.only.group)
					teks = `Lista De Los Dioses del grupo*${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
       ...
       /*case 'ownergrup':
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
				break*/
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedido recibido, chau infiel 👋 :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						...
						mentions(`Pedido recibido, chau infiel 👋 : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
		      ...
		      const cmd = body.slice(5)
                  ...
                  break
					...
					if (Number(args[0]) === 1) {
						...
						client.sendMessage(from,`Atención a todos los miembros hermosos activos de este grupo 📣\n\nEl antilink esta activo\n\nY solo los admins dioses podran pasar el enlace\n\nSi algun participante que no se admin se hace el vivo y envía un enlace de este grupo u otro grupo sera expulsado de este grupo de inmediato y mandado pa su casa`, text)
					} else if (Number(args[0]) === 0) {
						antilink.splice(from)
						fs.writeFileSync('./src/antilink.json', JSON.stringify(antilink))
						reply('❬ ✅ ❭ La funcion de antilink y anti legiovirgos esta deshabilitada en este grupo')
					} else {
			...
			case 'closegc':
					...
					const close = {
					text: `Grupo cerrado por el dios fiel @${nomor.split("@s.whatsapp.net")[0]}\nAhora *solo los dioses fieles* pueden enviar mensajes`,
                ...
                case 'bukagc':
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					open = {
					text: `Grupo abierto por el dios fiel @${sender.split("@")[0]}\nAhora *todos los participantes fieles e infieles* pueden enviar mensajes`,
					contextInfo: { mentionedJid: [sender] }
					}
		...
		case 'play':   
	        if (args.length < 1) return reply('Todo bien pero, Donde esta el nombre de la canción?')
		if (!isUser) return reply(mess.only.daftarB)
		...
		if (!isUser) return reply(mess.only.daftarB)
            ...
            break
                ...
                if (budy.includes(`hola`)) {
                  reply(`holaaa un gusto pa, soy Bot Emolai y vos?`)
                  }
		if (budy.includes(`todo bien`)) {
                  reply(`Si amigo todo bien, vite`)
                  }

		if (budy.includes(`quien sos`)) {
                  reply(`Soy el amor de tu vida bb`)
                  }
		...
		if (budy.includes(`sofi`)) {
                  reply(`que te pasa a vo con mi creadora gil? ojito que se te pudre el rancho`)
                  }
		...
		if (budy.includes(`Bot`)) {
                  reply(`Que queres ahora pedazo de infiel????`)
                  }
					
       if (budy.startsWith(`agrego`)) {
        const none = fs.readFileSync('./mp3/agrego.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
	if (budy.startsWith(`boludo`)) {
        const none = fs.readFileSync('./mp3/boludo.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
	if (budy.startsWith(`buen dia`)) {
        const none = fs.readFileSync('./mp3/buen dia.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
	if (budy.startsWith(`copa`)) {
        const none = fs.readFileSync('./mp3/copa.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
	if (budy.startsWith(`enano`)) {
        const none = fs.readFileSync('./mp3/enano.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
	if (budy.startsWith(`leche`)) {
        const none = fs.readFileSync('./mp3/leche.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
	if (budy.startsWith(`messi`)) {
        const none = fs.readFileSync('./mp3/messi.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
	if (budy.startsWith(`pasion`)) {
        const none = fs.readFileSync('./mp3/pasion.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
	if (budy.startsWith(`puto`)) {
        const none = fs.readFileSync('./mp3/puto.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
	if (budy.startsWith(`sagitario`)) {
        const none = fs.readFileSync('./mp3/sagitario.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
	if (budy.startsWith(`Ya me voy a dormir`)) {
        ...
        if (budy.startsWith(`tarado`)) {
        const none = fs.readFileSync('./mp3/tarado.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
	if (budy.startsWith(`torta`)) {
        const none = fs.readFileSync('./mp3/torta.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
	if (budy.startsWith(`xq`)) {
        const none = fs.readFileSync('./mp3/xq.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
